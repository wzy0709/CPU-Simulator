	...
	while 1:
		...
		if psort and current_time == psort[0].arrival_time:
			...
			if current_time < printime:
				print("time %ims: Process %c arrived; added to ready queue %s" % (
				current_time, new_p.name, get_string(cpu_q)))
		current_time += 1
		# cpu process
		if not cpu_working:
			if len(cpu_q) > 1:
...
def SJF(processes):
	...
	while 1:
		# grab first process in the que  (grab the first process in ready queue)
		...
		if curr_time == term_time:
			...
			if len(curr_proc.bursts) == 0:  # (Process terminates)
				print("time %ims: Process %c terminated %s" \
			...
			else:  # next small task in process
				if curr_time <= 1000:
		...
		if curr_time in io_proc:  # waiting -> ready
		...
		if len(sort_proc) == 0 and cpu_q == [] and curr_proc == None and cpu_running == False and io_proc == {}:
			break
		curr_time += 1
...
def SRT(processes):
	...
	switch = -1
	for p in sort_proc:
		tmp_p[p.name] = p.bursts
		tmp_est[p.name] = p.est_time
	print("time %ims: Simulator started for term_timeSRT %s" \
	      % (curr_time, get_string(cpu_q)))

	while 1:
		if switch > 0 and curr_proc != None:
			cpu_q.append(curr_proc)
			cpu_q = sorted(cpu_q, key=lambda x: (x.est_time, x.name))
			curr_proc = None
		...
		if cpu_q != [] and curr_proc == None \
				and cpu_time - curr_time <= 1:
			curr_proc = cpu_q.pop(0)
			remain_time = curr_proc.est_time
			# term_time = curr_time + curr_proc.bursts[0].cpu_time
			if curr_time >= cpu_time:
				cpu_time = curr_time
				cpu_time += context_switch_time / 2 - 1
				switch = context_switch_time / 2 - 1
		# exchange two processes
		...
		if not cpu_running and curr_time >= cpu_time and curr_proc != None:
			...
			print("time %ims: Process %c (tau %ims) started using the CPU for %ims burst %s" \
			      % (
			      curr_time, curr_proc.name, tmp_est[curr_proc.name], curr_proc.bursts[0].cpu_time, get_string(cpu_q)))
			context_switch += 1
			run_time = 0
			term_time = curr_time + curr_proc.bursts[0].cpu_time
			burst_list.append(curr_proc.bursts[0].cpu_time)

		if cpu_q != [] and (remain_time > cpu_q[0].est_time) and curr_proc != None and switch <= 0:

			cpu_running = False
			curr_proc.bursts[0].cpu_time -= run_time
			run_time = 0
			curr_proc.est_time = tmp_est[curr_proc.name] - curr_proc.bursts[0].remain + curr_proc.bursts[0].cpu_time
			if curr_proc.est_time < 0: curr_proc.est_time = 0
			tmp_pname = curr_proc.name
			if curr_time >= cpu_time:
				cpu_time = curr_time
				cpu_time += context_switch_time
				switch = context_switch_time
				print("time %ims: Process %c (tau %ims) will preempt %s %s" \
				      % (curr_time, proc.name, tmp_est[proc.name], tmp_pname, get_string(cpu_q)))
			else:
				print(1111111)
				print("time %ims: Process %c (tau %ims) completed I/O; added to ready queue %s" \
				      % (curr_time, proc.name, tmp_est[proc.name], get_string(cpu_q)))
			cpu_q.append(curr_proc)
			curr_proc = cpu_q.pop(0)
			term_time = curr_time + curr_proc.bursts[0].cpu_time + context_switch_time
			cpu_q = sorted(cpu_q, key=lambda x: (x.est_time, x.name))
			remain_time = curr_proc.est_time
			context_switch += 1
		# to do: edit cpu time
		# check if process terminated(no more burst)
		# if not, keep going with i/o
		# print(term_time,curr_time)
		if term_time == curr_time:

			cpu_running = False
			curr_burst2 = tmp_p[curr_proc.name]
			k = -len(curr_proc.bursts)
			run_time = 0
			curr_burst = curr_burst2[k]

			curr_proc.bursts.pop(0)
			if len(curr_proc.bursts) == 0:  # (Process terminates)
				print("time %ims: Process %c terminated %s" \
				      % (curr_time, curr_proc.name, get_string(cpu_q)))
				curr_proc = None
				...
				print("time %ims: Process %c (tau %ims) completed a CPU burst; %i bursts to go %s" \
				      % (curr_time, curr_proc.name, tmp_est[curr_proc.name], len(curr_proc.bursts), get_string(cpu_q)))
				old_tau = tmp_est[curr_proc.name]

				tmp_est[curr_proc.name] = math.ceil(alpha * curr_burst.remain + (1 - alpha) * old_tau)
				curr_proc.est_time = tmp_est[curr_proc.name]
				print("time %ims: Recalculated tau from %ims to %ims for process %c %s" \
				      % (curr_time, old_tau, tmp_est[curr_proc.name], curr_proc.name, get_string(cpu_q)))
				io_time = curr_time + curr_burst.io_time + context_switch_time / 2
				print("time %ims: Process %c switching out of CPU; will block on I/O until time %ims %s]" \
				      % (curr_time, curr_proc.name, io_time, get_string(cpu_q)))
		...
		if curr_time in io_proc:  # waiting -> ready
			for proc in io_proc[curr_time]:
				cpu_q.append(proc)
				cpu_q = sorted(cpu_q, key=lambda x: (x.est_time, x.name))
				if curr_proc != None and remain_time >= cpu_q[0].est_time and switch <= 0:
					cpu_running = False
					curr_proc.bursts[0].cpu_time -= run_time

					run_time = 0
					curr_proc.est_time = tmp_est[curr_proc.name] - curr_proc.bursts[0].remain + curr_proc.bursts[
						0].cpu_time
					if curr_proc.est_time < 0: curr_proc.est_time = 0
					tmp_pname = curr_proc.name
					if curr_time >= cpu_time:
						...
						cpu_time += context_switch_time
						switch = context_switch_time
						print("time %ims: Process %c (tau %ims) completed I/O; preempting %s %s" \
						      % (curr_time, proc.name, tmp_est[proc.name], tmp_pname, get_string(cpu_q)))
					else:
						print("time %ims: Process %c (tau %ims) completed I/O; added to ready queue %s" \
						      % (curr_time, proc.name, tmp_est[proc.name], get_string(cpu_q)))
	...
	print("time %ims: Simulator ended for SRT [Q empty]" % (curr_time + 2))
...
if (__name__ == "__main__"):
	...
	print('')
	ret = SRT(copy.deepcopy(processes))
	print(ret)
	print('')
